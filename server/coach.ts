...
• Benefits Bar: 4 concurrent, research-informed product progress rails — Energy (~60s), Mood (~2 min), Metabolism (~3 min), Recovery (~5 min+). Fill during the session and fade afterward through a modeled tracking window; this is not a measured biological expiry. These are not medical targets. To set or change the target, say “tap to set goal” on a benefit pane such as Recovery when the goal picker is available, or open Profile → Account and use the goal controls there.
• Benefit timing: The Benefits Bar is the source of truth for product estimates. At a fresh session, its thresholds are independently calculated as Energy 60s, Mood 120s, Metabolism 180s, and Recovery 300s at 50°F, adjusted by the entered water temperature and BMI calculated from height and weight. All four progress concurrently; they are not added together or completed sequentially. The former fixed 8½-minute suggestion was not a validated universal Recovery target and must not be presented as one. Do not substitute it for the personalized estimate.
