 ...
 * Mirrors the trendingUpNudge() function in TryThisNextCard.tsx exactly.
 *
 * Temperature tiers (°F):
...
export function deriveNudgeForPush(plunges: Plunge[]): NudgePayload | null {
  ...
  if (delta !== null) {
    if (delta > 0.05) {
      // Use personalised temperature-aware copy when recent averages are available,
      // falling back to the generic shared message when they are not.
      ...
      const icon = recents && recents.avgTempF <= 45 ? "🏆" : "📈";
