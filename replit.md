...
- **Music Widget (May 2026)**: `client/src/components/MusicWidget.tsx` is a slim full-width "music bar" pinned at the top of the timer screen (just under the header, auto-shifts down when the email-verify banner is present). Single row: service icon + native `<select>` dropdown of curated presets (Cold Plunge Focus, Wim Hof Breathing, Deep Focus, Workout Beast Mode) plus "Paste custom Spotify/Apple Music URL…" and "Clear current playlist" entries + Play button + ⚙ Settings (autoplay toggle + custom URL paste form lives in the modal). Selection + autoplay preference stored in localStorage under `coldstreak-music-config`. Two playback modes: (1) **Auto-play with timer** — when enabled, `handleStart()` calls `openMusic()` AFTER the timer is confirmed started (so a bailed-out countdown with zero duration won't launch music), `window.open`-ing the Spotify/Apple Music deep link; (2) **Free listen mode** — the bar's Play button always opens the chosen playlist on demand without starting a timer. Service is auto-detected from the URL (spotify.com / spotify: → Spotify; music.apple.com / music: → Apple Music). Stopping playback is delegated to the host music app since cross-app playback control isn't available from the WebView.
