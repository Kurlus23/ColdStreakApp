...
- **Currently supported model: Inkbird IBS-TH2 Plus** (waterproof external probe). As of build 19 we read it via **BLE advertisement scanning** (no GATT connection), modeled on Home Assistant's `inkbird-ble` library. The device broadcasts manufacturer-specific data every ~1–2s: bytes 0–1 = signed int16 LE ÷ 100 = °C, byte 6 = external probe flag, byte 7 = battery %. Build 18 and earlier mistakenly subscribed to the standard Health Thermometer GATT service (`0x1809` / `0x2A1C`) — IBS-TH2 Plus does not actually implement that profile, which is why every reading came back as ~45°F garbage regardless of probe state. The beacon implementation lives in `client/src/pages/Home.tsx` (`parseTempFromBytes`, `inspectScanResultPayloads`, `detectThermoProtocol`, `startThermoStream`, `startThermoKeepalive`, `autoReconnectThermo`). A keepalive watchdog restarts the scan if no broadcast arrives for 15s (handles the case where HR scanner pre-empts the global LE scan slot) and escalates to `autoReconnectThermo` after 3 silent restart attempts (~45s). Selected after retiring ThermoPro TP25 reverse-engineering work — TP25 used a proprietary protocol (`1086fff0…`) that never returned reliable probe data.
  ...
  **Off-brand thermometer diagnostics (build 20 JS, Apr 2026)**: Discovered an off-brand probe (advertised name "sps", manufacturer ID 2690 / 0x0A82 — same ID as Inkbird) that broadcasts a different byte layout than IBS-TH2 Plus. Its 7-byte payload `43 13 01 74 09 64 08` decodes to 49.31 °C / 120.8 °F under the stock Inkbird parser (bytes 0-1 LE), which then got silently squashed to 60 °F by an old `Math.min(60, Math.max(25, …))` clamp at every `setTemperature()` call site, hiding the parser bug. Three diagnostic upgrades shipped: (1) `clampDisplayTempF()` helper replaces the inline clamp, widens the display range to 25–90 °F (so a probe sitting in a warm room reads correctly), and *records* every clamp event so the panel can warn instead of silently lying; (2) `listTempCandidates()` enumerates every plausible temperature interpretation of the bytes (every offset, both endiannesses, °C and °F) within the −10…80 °C plausible range — added as an expandable "All candidate temps" section in the diagnostic panel so the user can match against the device's own screen reading to identify the correct byte offset for an unknown protocol; (3) the diagnostic panel now also surfaces device name, deviceId, and RSSI so off-brand units are obvious at a glance. Red ⚠ banner appears in the panel when clamping is occurring ("Parser read 121°F, clamped to 60°F for display").
