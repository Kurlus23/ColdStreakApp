...
- **Currently supported model: Inkbird IBS-TH2 Plus** (waterproof external probe). As of build 19 we read it via **BLE advertisement scanning** (no GATT connection), modeled on Home Assistant's `inkbird-ble` library. The device broadcasts manufacturer-specific data every ~1–2s: bytes 0–1 = signed int16 LE ÷ 100 = °C, byte 6 = external probe flag, byte 7 = battery %. Build 18 and earlier mistakenly subscribed to the standard Health Thermometer GATT service (`0x1809` / `0x2A1C`) — IBS-TH2 Plus does not actually implement that profile, which is why every reading came back as ~45°F garbage regardless of probe state. The beacon implementation lives in `client/src/pages/Home.tsx` (`parseTempFromBytes`, `inspectScanResultPayloads`, `detectThermoProtocol`, `startThermoStream`, `startThermoKeepalive`, `autoReconnectThermo`). A keepalive watchdog restarts the scan if no broadcast arrives for 15s (handles the case where HR scanner pre-empts the global LE scan slot) and escalates to `autoReconnectThermo` after 3 silent restart attempts (~45s). Selected after retiring ThermoPro TP25 reverse-engineering work — TP25 used a proprietary protocol (`1086fff0…`) that never returned reliable probe data.
  ...
  **Off-brand thermometer support (build 20 JS, Apr 2026)**: Discovered an off-brand probe (advertised name "sps") that pretends to be an Inkbird but uses a *completely different* encoding: it packs the temperature into the FIRST TWO BYTES of the raw BLE manufacturer-data block, which the BLE spec (and therefore the Capacitor BLE plugin) interprets as the company identifier (uint16 LE). The temperature is then `mfrId / 100` in °C — confirmed against two real captures: thermometer screen shows 80.4°F → mfrId 2690 → 26.90°C → 80.42°F ✓, and screen shows 93°F → mfrId 3380 → 33.80°C → 92.84°F ✓ (the mfrId varies as the temp changes — it isn't a real fixed company ID). The `decodeMfrIdAsTempF()` helper does the conversion. In `inspectScanResultPayloads`, after the standard Inkbird decode runs, if the resulting probe temp is outside a believable 23–113°F range AND the mfrId-as-temp gives a sane value, the parser auto-falls-back to the mfrId reading. This makes off-brand "sps" units Just Work without device-name special cases. The earlier display-range clamp `Math.min(60, Math.max(25, …))` that was silently squashing readings to 60°F was widened to 25–90°F via `clampDisplayTempF()` so a probe sitting in a warm room shows its real reading. The reverse-engineering diagnostic panel (yellow BLE-payload box with raw bytes, candidate-temp list, RSSI, clamp warnings) was used during the off-brand investigation and then removed once the auto-fallback was confirmed working — git history `commit 8c4f73`/`commit 6368a2` has it if a future off-brand model needs the same treatment.
