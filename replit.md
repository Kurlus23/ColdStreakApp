...
- **Music Widget (May 2026)**: `client/src/components/MusicWidget.tsx` adds a Spotify/Apple Music widget to the timer pane. Users pick a curated preset (Cold Plunge Focus, Wim Hof Breathing, Deep Focus, Workout Beast Mode) or paste any Spotify/Apple Music URL; selection + autoplay preference are stored in localStorage under `coldstreak-music-config`. Two playback modes: (1) **Auto-play with timer** — when enabled, `handleStart()` calls `openMusic()` (just-in-time `window.open` to the Spotify/Apple Music deep link) so playback launches automatically when the plunge starts; (2) **Free listen mode** — the widget's Play button always opens the chosen playlist on demand without starting a timer. Service is auto-detected from the URL (spotify.com / spotify: → Spotify; music.apple.com / music: → Apple Music). Stopping playback is delegated to the host music app (Spotify/Apple Music) since cross-app playback control isn't available from the WebView.
