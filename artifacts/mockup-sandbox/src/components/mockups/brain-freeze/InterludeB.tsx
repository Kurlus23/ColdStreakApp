export function InterludeB() {
  return (
    ...
    >
      <div className="pointer-events-none absolute -left-28 -top-32 h-[300px] w-[300px] rounded-full bg-cyan-400/[0.06] blur-3xl" />
      <div className="pointer-events-none absolute -bottom-40 -right-36 h-[400px] w-[400px] rounded-full bg-cyan-400/[0.06] blur-3xl" />

      <header className="relative pt-[56px] text-center">
      ...
      <div
        className="absolute right-5 top-[52px] flex items-center gap-2 rounded-full border border-cyan-300/50 bg-cyan-400/[0.15] px-3 py-1.5 backdrop-blur-sm"
        style={{ boxShadow: "0 0 12px rgba(34,211,238,0.3)" }}
        aria-label="3 brain freezes"
      >
        <span className="text-[16px] leading-none">🧠</span>
        <span className="font-mono text-[22px] font-bold leading-none text-cyan-300">3</span>
      </div>

      <main className="absolute inset-0">
        <section className="absolute left-1/2 top-[190px] flex h-[280px] w-[280px] -translate-x-1/2 items-center justify-center">
          <svg className="absolute inset-0" viewBox="0 0 280 280" fill="none" aria-hidden="true">
            <defs>
              <filter id="interlude-b-arc-glow" x="-50%" y="-50%" width="200%" height="200%">
            ...
            <circle
              ...
              filter="url(#interlude-b-arc-glow)"
            />
            <circle cx="241.8" cy="214.1" r="5" fill="#67e8f9" filter="url(#interlude-b-arc-glow)" />
          </svg>
          <div className="relative text-center">
            ...
            <div className="mt-3 text-[10px] font-medium tracking-[0.2em] text-slate-400">ELAPSED</div>
          </div>
        </section>

        <section className="absolute left-5 right-5 top-[490px] rounded-xl border border-cyan-300/20 bg-cyan-400/[0.06] px-4 py-3 backdrop-blur-sm">
      ...
      <footer className="absolute bottom-10 left-6 right-6 flex flex-col gap-4">
        ...
        <div>
          <div className="flex h-2 gap-1.5">
            <div className="h-full flex-1 rounded-full border border-cyan-300/75 bg-cyan-300 shadow-[0_0_10px_rgba(34,211,238,.65)]" />
            <div className="h-full flex-1 rounded-full border border-amber-300/0 bg-amber-900/[0.18]">
              <div className="h-full w-[70%] rounded-full bg-amber-400" />
            </div>
            <div className="h-full flex-1 rounded-full bg-orange-800/[0.44]" />
            <div className="h-full flex-1 rounded-full bg-emerald-900/[0.44]" />
          </div>
          <div className="mt-1 flex justify-between text-[9px] text-slate-400">
            <span>⚡ Energy</span>
            <span>😊 Mood</span>
            <span className="text-slate-600">🔥 Metabolism</span>
        ...
        <button
          type="button"
          className="h-[68px] w-full rounded-2xl bg-gradient-to-b from-red-500 to-red-700 text-[20px] font-bold text-white"
